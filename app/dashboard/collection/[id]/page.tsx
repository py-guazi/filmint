...
export default function CollectionPage() {
  ...
  const { mints, loading: mintsLoading } = useMints(collectionId)
            ...
            <div className="md:col-span-2">
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div>
                  ...
                  <div className="font-medium">{collection.itemCount}</div>
                </div>
                <div>
                  <div className="text-sm text-muted-foreground">Created</div>
                  <div className="font-medium">
                    {collection.createdAt instanceof Date
                      ? collection.createdAt.toLocaleDateString(undefined, { year: "numeric", month: "long" })
              ...
              <div className="flex gap-4">
                ...
                <Button variant="outline">View on Filfox</Button>
              </div>
            </div>
          </div>
        ) : (
        ...
        <Tabs defaultValue="items" className="w-full">
          ...
          <TabsContent value="items">
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {mintsLoading ? (
                <div>Loading mints...</div>
              ) : (
                mints.map((mint) => (
                  <Card key={mint.id} className="overflow-hidden hover:shadow-lg transition-shadow">
                    <div className="aspect-square relative">
                      <Image src={mint.image || "/placeholder.svg"} alt={mint.name} fill className="object-cover" />
                    </div>
                    <CardHeader>
                      <CardTitle className="text-lg">{mint.name}</CardTitle>
                      <CardDescription className="line-clamp-2">{mint.description}</CardDescription>
                    </CardHeader>
                    <CardFooter className="flex justify-between">
                      <div className="text-sm text-muted-foreground">Token ID: {mint.tokenId}</div>
